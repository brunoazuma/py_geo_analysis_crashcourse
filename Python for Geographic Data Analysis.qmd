...
Esta aula será dividida em quatro partes:
...
tabela.drop(columns="Pop. Urb. 2022")\
    .sort_values("% Pop. Rural 2022", ascending=False).head(4)
```
...
cols = ["Posto", "Distrito", "Pop. Total 2022", "Posto_numerico"]
tabela[cols].head(3)
```
...
print(f"A tabela de escolas possui {escolas.shape[0]} linhas e {escolas.shape[1]} colunas.")
escolas.head(3)
```
...
# Exibe as primeiras linhas da tabela com a geometria
pontos_onibus.drop(columns=['id']).head()
```
...
distritos.drop(columns=['id']).head(2)
```
...
pontos_onibus_distritos = pontos_onibus\
                          .sjoin(distritos[colunas_selecionadas],
                          predicate="within")
...
Agora temos uma tabela com os dados de população e o número de pontos de ônibus por distrito, o que nos permitirá realizar análises mais aprofundadas sobre a relação entre esses dois aspectos.

```{python}
#| echo: true

cols = ["Posto", "Distrito", "Pop. Total 2022", "Número de Pontos de Ônibus"]
tabela_distritos[cols].head(3)
```
...
tabela_distritos.head(3)
```

# Visualização de dados geográficos com Python
...
cols = ["DISTRITO", "Pontos de Ônibus por 100.000 habitantes", "geometry"]

distritos_com_pontos[cols].head(4)
```
